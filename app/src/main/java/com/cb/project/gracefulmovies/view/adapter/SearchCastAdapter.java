...
public class SearchCastAdapter extends BaseRecyclerAdapter<MovieSearchModel.CastInfo, SearchCastAdapter.CastVH> {
    ...
    protected CastVH onCreate(LayoutInflater inflater, ViewGroup parent, int viewType) {
        return new CastVH(inflater.inflate(com.cb.project.gracefulmovies.R.layout.item_search_cast, parent, false));
    }
    ...
    protected void onBind(final CastVH holder, int position) {
        ...
        Glide.with(mContext).load(cast.image).crossFade().error(com.cb.project.gracefulmovies.R.drawable.svg_ic_loading).into(holder.castImage);
    ...
    class CastVH extends BaseRecyclerViewHolder {
        ...
        CastVH(View itemView) {
            ...
            castImage = findView(com.cb.project.gracefulmovies.R.id.cast_img);
            castText = findView(com.cb.project.gracefulmovies.R.id.cast_text);
        }
    }
