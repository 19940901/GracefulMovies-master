...
public class NightModeFragment extends SideFragment {
    ...
    @Override
    void displayUI() {
        mCollapsingToolbarLayout.setContentScrimResource(com.cb.project.gracefulmovies.R.color.md_night_primary);

        mToolbar.setTitle(getString(com.cb.project.gracefulmovies.R.string.night_mode));

        mFab.setBackgroundTintList(createColorStateList(com.cb.project.gracefulmovies.R.color.md_night_primary));
        ...
        if (mHandler == null) {
            ...
            mHandler.postDelayed(new Runnable() {
                @Override
                public void run() {
                    Glide.with(getActivity()).load(com.cb.project.gracefulmovies.R.drawable.pic_night).into(mHeaderImg);
        ...
        mNestedScrollView.setBackgroundResource(com.cb.project.gracefulmovies.R.color.color_window_background_night);

        mPrimaryColorImg.setVisibility(View.INVISIBLE);

        mDayNightModeText.setTextColor(Color.WHITE);
        mPrimaryColorText.setTextColor(ContextCompat.getColor(getContext(), com.cb.project.gracefulmovies.R.color.color_disable));
